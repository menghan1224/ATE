...
public class SignalViewEditorInputFactory implements IElementFactory {
	...
	public IAdaptable createElement(IMemento memento) {
		...
		try {
			if (obj instanceof ATESignalTypeView) {
				return new SignalViewEditorInput((ATESignalTypeView) obj); // 这个创建编辑器的输入数据。
			} else {
				return null;
			}
		} catch (LMFResourceException ex) {
