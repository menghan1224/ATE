...
public class VoltSentence implements TableAteViewRow{
	private TextCell pin1Cell;
	private TextCell pin2Cell;
	private TextCell voltParaCell;
	...
	private DataBinding voltParaBinding;
	...
	List<TextCell> cellList=new ArrayList<TextCell>();
	{
		label1=new RowLabel(){
			...
			setIconSpacing(3);
			setIcon(RUCMPluginResource.getImage("volt.png"));
...
//				getTextView().setDefaultTextColor(TextTableViewRow.LABEL_TEXT_COLOR);
	...
	public VoltSentence(final TableAteView table,AteSentence sentence) {
...
//						cell.setCaretPosition(CaretIndex.before(cell.textLength() - text.length()));
			...
			voltParaCell=new TextCell(defaultDelegate, false, false);
			voltParaCell.setText("");
			voltParaCell.setHintText("CUREENT_PARAMETER");
			voltParaCell.setBreakIterator(null);
			...
			list.add(pin2Cell);
			list.add(label3);
			list.add(voltParaCell);
			list.add(label4);
			cellList.add(pin1Cell);
			cellList.add(pin2Cell);
			cellList.add(voltParaCell);
	...
	public void tableViewRowAdded(TableAteView table) {
		...
		voltParaBinding=new DataBinding(voltParaCell,TextCell.TEXT_CHANGED,sentence,AteSentence.KEY_NAME) {
			
			@Override
			protected void viewPropertyChanged(Object arg) {
				// TODO Auto-generated method stub
				sentence.setPara3(voltParaCell.getPlainText());
			}
			
			@Override
			protected void modelPropertyChanged(ManagedObject value) {
				// TODO Auto-generated method stub
				voltParaCell.setText(sentence.getPara3());
			}
		};
