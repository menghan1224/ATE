...
public class CurrentSentence implements TableAteViewRow{
	private TextCell pin1Cell;
	private TextCell pin2Cell;
	private TextCell currentParaCell;
	...
	private DataBinding currentParaBinding;
	...
	List<TextCell> cellList=new ArrayList<TextCell>();
	{
		label1=new RowLabel(){
			...
			setIconSpacing(3);
			setIcon(RUCMPluginResource.getImage("current.png"));
...
//				getTextView().setDefaultTextColor(TextTableViewRow.LABEL_TEXT_COLOR);
	...
	public CurrentSentence(final TableAteView table,AteSentence sentence) {
...
//						cell.setCaretPosition(CaretIndex.before(cell.textLength() - text.length()));
			...
			currentParaCell=new TextCell(defaultDelegate, false, false);
			currentParaCell.setText("");
			currentParaCell.setHintText("CUREENT_PARAMETER");
			currentParaCell.setBreakIterator(null);
			...
			list.add(pin2Cell);
			list.add(label3);
			list.add(currentParaCell);
			list.add(label4);
			cellList.add(pin1Cell);
			cellList.add(pin2Cell);
			cellList.add(currentParaCell);
	...
	public void tableViewRowAdded(TableAteView table) {
		...
		currentParaBinding=new DataBinding(currentParaCell,TextCell.TEXT_CHANGED,sentence,AteSentence.KEY_NAME) {
			
			@Override
			protected void viewPropertyChanged(Object arg) {
				// TODO Auto-generated method stub
				sentence.setPara3(currentParaCell.getPlainText());
			}
			
			@Override
			protected void modelPropertyChanged(ManagedObject value) {
				// TODO Auto-generated method stub
				currentParaCell.setText(sentence.getPara3());
			}
		};
