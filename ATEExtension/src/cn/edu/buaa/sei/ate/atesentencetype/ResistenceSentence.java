...
public class ResistenceSentence implements TableAteViewRow{
	private TextCell pin1Cell;
	private TextCell pin2Cell;
	private TextCell resisParaCell;
	...
	private DataBinding resisParaBinding;
	...
	List<TextCell> cellList=new ArrayList<TextCell>();
	{
		label1=new RowLabel(){
			...
			setIconSpacing(3);
			setIcon(RUCMPluginResource.getImage("resis.png"));
...
//				getTextView().setDefaultTextColor(TextTableViewRow.LABEL_TEXT_COLOR);
	...
	public ResistenceSentence(final TableAteView table,AteSentence sentence) {
...
//						cell.setCaretPosition(CaretIndex.before(cell.textLength() - text.length()));
			...
			resisParaCell=new TextCell(defaultDelegate, false, false);
			resisParaCell.setText("");
			resisParaCell.setHintText("RESISTENCE_PARAM");
			resisParaCell.setBreakIterator(null);
			list.add(label1);
			list.add(pin1Cell);
			list.add(label2);
			list.add(pin2Cell);
			list.add(label3);
			list.add(resisParaCell);
			list.add(label4);
			cellList.add(pin1Cell);
			cellList.add(pin2Cell);
			cellList.add(resisParaCell);
	...
	public void tableViewRowAdded(TableAteView table) {
		...
		resisParaBinding=new DataBinding(resisParaCell,TextCell.TEXT_CHANGED,sentence,AteSentence.KEY_NAME) {
			
			@Override
			protected void viewPropertyChanged(Object arg) {
				// TODO Auto-generated method stub
				sentence.setPara3(resisParaCell.getPlainText());
			}
			
			@Override
			protected void modelPropertyChanged(ManagedObject value) {
				// TODO Auto-generated method stub
				resisParaCell.setText(sentence.getPara3());
			}
		};
