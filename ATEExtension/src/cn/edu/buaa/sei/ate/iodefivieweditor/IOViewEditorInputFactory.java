...
public class IOViewEditorInputFactory implements IElementFactory {
	...
	public IAdaptable createElement(IMemento memento) {
		...
		try {
			if (obj instanceof ATEIODefinitionView) {
				return new IOViewEditorInput((ATEIODefinitionView) obj); // 这个创建编辑器的输入数据。
			} else {
				return null;
			}
		} catch (LMFResourceException ex) {
