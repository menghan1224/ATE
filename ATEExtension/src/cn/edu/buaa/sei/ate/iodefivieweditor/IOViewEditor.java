...
public class IOViewEditor extends SnailLMFEditorBase{
	private ViewContext viewContext;
	private FaultEditorView view;
	@Override
	protected ViewContext createPartControl(Container container) {
		// TODO Auto-generated method stub
		ATEIODefinitionView ATEIODefinitionView =((IOViewEditorInput) getEditorInput()).getFault();
		view = new FaultEditorView(ATEIODefinitionView); // 创建GUI
...
class FaultEditorView extends SnailEditorView {
	...
	private final ATEIODefinitionView model;
	
	private final SectionScrollView scrollView;
	
	FaultEditorView(final ATEIODefinitionView model) { // 把当前打开的Fault对象传进来
		this.model = model;
...
class NameAndDescriptionSection extends SectionView {
	...
	NameAndDescriptionSection(ATEIODefinitionView model) { // 把当前Actor传进来
		this.model = model;
		
		// 创建一个Table
		table = new TableAteView(6);
	...
	private void rebuildRows() {
		table.removeAllRows();
		table.addRow(new IOTitleRow());
		if(model.getIodefinitions()==null||model.getIodefinitions().size()==0){
			IODefinition definition=nullFactory.createIODefinition();
			model.getIodefinitions().add(definition);
			table.addRow(new IORow(model, definition, table));
		}
		else{
			for(IODefinition defi:model.getIodefinitions()){
				table.addRow(new IORow(model, defi, table));
			}
		}
